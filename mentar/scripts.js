...
function generate_problem(){
	let no_checkbox_is_checked = true;
	let used_operators = [];
	for (let i = 0; i < 3; i++) {
		let chbox = document.getElementById('cbxOp' + i) 
		if (chbox.checked){
			used_operators.push(chbox.value);
			no_checkbox_is_checked = false;
		}
	}
	if (no_checkbox_is_checked) {
...
btn_generate.addEventListener('mouseup', function(){
	generate_problem();
});
...
btn_submit.addEventListener('keyup', function(e){
	if (e.code === 'Enter') {
		submit_answer();
	}
});

btn_options.addEventListener('mouseup', function(){
...
// Надо вынести код кнопки submit в отдельную функцию
